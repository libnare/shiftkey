...
<div :class="[$style.root, { '_forceShrinkSpacer': deviceKind === 'smartphone' }]">
	...
	<div :class="$style.nonTitlebarArea">
		<XSidebar v-if="!isMobile" :class="$style.sidebar" :isNoteUI="true" :showWidgetButton="!isDesktop" @widgetButtonClick="widgetsShowing = true"/>
		...
		<div :class="[$style.contents, !isMobile && prefer.r.showTitlebar.value ? $style.withSidebarAndTitlebar : null]" @contextmenu.stop="onContextmenu">
			...
			<div v-if="isNotePage" :class="$style.noteRoot">
				<div class="_spacer" style="--MI_SPACER-w: 800px;">
					<MkPostForm :class="$style.postForm" class="post-form _panel" fixed style="margin-bottom: var(--MI-margin);"/>
				</div>
			</div>
			<StackingRouterView v-if="prefer.s['experimental.stackingRouterView'] && !isNotePage" :class="$style.content"/>
			<RouterView v-else-if="!isNotePage" :class="$style.content"/>
...
const isNotePage = computed(() => mainRouter.currentRoute.value.path === '/');
